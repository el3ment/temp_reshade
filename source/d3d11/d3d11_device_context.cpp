...
void D3D11DeviceContext::track_active_rendertargets(UINT NumViews, ID3D11RenderTargetView *const *ppRenderTargetViews, ID3D11DepthStencilView *pDepthStencilView)
{
	...
	{
		UINT actual_numviews = 0;
		if (ppRenderTargetViews != nullptr) {
			// calculate the "actual" number of views
			// sometimes numviews is set to 4, but only
			// a few rendertargets have valid pointers
		...
		_draw_call_tracker.track_rendertargets(pDepthStencilView, nullptr, actual_numviews, ppRenderTargetViews);
	}
}
